 ...
 * @author Admin
 */
public class Sale_CreatePartServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String purchaseprice = request.getParameter("partpurchaseprice");
            String retailprice = request.getParameter("partretailprice");
            HttpSession s = request.getSession();
            SalePerson sale = (SalePerson) s.getAttribute("Sale");
            if (sale != null) {
                if (name != null) {
                    PartsDAO pd = new PartsDAO();
                    int result = pd.createPart(name, purchaseprice, retailprice);
                    if (result > 0) {
                        ArrayList<Parts> listParts = pd.getAllParts();
                        if (listParts != null && !listParts.isEmpty()) {
                            s.setAttribute("SALE_LISTPARTS", listParts);
                        }
                        request.setAttribute("SUCCESS", "Create successfully!");
                        request.getRequestDispatcher("MainServlet?action=sale_createPartPage").forward(request, response);
                    } else {
