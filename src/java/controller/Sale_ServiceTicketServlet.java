 ...
 * @author Admin
 */
public class Sale_ServiceTicketServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(sale != null) {
                ...
                ArrayList<ServiceTicket> listSt = std.getAllServiceTicket();
                if(listSt != null && !listSt.isEmpty()) {
                    s.setAttribute("SALE_LISTST", listSt);
                    response.sendRedirect("MainServlet?action=sale_ServiceTicketPage");
                } else {
                    request.setAttribute("ERROR", "Something wrong. Try again!");
                    request.getRequestDispatcher("MainServlet?action=salePersonPage").forward(request, response);
                }
            } else {
