 ...
 * @author Admin
 */
public class Sale_CreateInvoiceServlet extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(sale != null) {
                if(carid != null && custid != null) {
                    SalesInvoiceDAO sid = new SalesInvoiceDAO();
                    int result = sid.createInvoice(sale.getId(), carid, custid);
                    if (result > 0) { 
                        WishlistDAO wld = new WishlistDAO();
                        ArrayList<WishList> list = wld.getWishlist(custid);
                        s.setAttribute("WISHLIST", list);
                        request.setAttribute("SUCCESS", "Create successfully!");
                        request.getRequestDispatcher("MainServlet?action=sale_InvoicePage").forward(request, response);
                    } else {
