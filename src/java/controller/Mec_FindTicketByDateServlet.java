 ...
 * @author Admin
 */
public class Mec_FindTicketByDateServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String dateReceive = request.getParameter("date").trim();
            HttpSession s = request.getSession();
            Mechanic mecha = (Mechanic) s.getAttribute("Mecha");
            if(mecha != null) {
                ServiceTicketDAO std = new ServiceTicketDAO();
                ArrayList<ServiceTicket> listSt = std.searchTicketByDate(mecha.getId(), dateReceive);
