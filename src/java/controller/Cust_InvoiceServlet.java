 ...
 * @author Admin
 */
public class Cust_InvoiceServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (user != null) {
                ...
                ArrayList<Invoice> list = id.getInvoices("" + user.getCustid(), date);
                if (list != null && !list.isEmpty()) {
                    s.setAttribute("LIST_INVOICE", list);
                    request.getRequestDispatcher("MainServlet?action=custDashboard&FindDate=" + date).forward(request, response);
                } else {
                ...
                request.setAttribute("ERROR1", "Something wrong. Try again!");
