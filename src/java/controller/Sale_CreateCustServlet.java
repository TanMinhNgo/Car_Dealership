 ...
 * @author Admin
 */
public class Sale_CreateCustServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (sale != null) {
                if (custname != null && custphone != null && custaddress != null) {
                    CustomerDAO cd = new CustomerDAO();
                    int result = cd.createCustomer(custname, custphone, custsex, custaddress);
                    if (result > 0) {
                        ArrayList<Customer> listCust = cd.getCustomers();
                        if (listCust != null && !listCust.isEmpty()) {
                            s.setAttribute("SALE_LISTCUST", listCust);
                        }
                        request.setAttribute("SUCCESS", "Create successfully!");
                        request.getRequestDispatcher("MainServlet?action=sale_createCustPage").forward(request, response);
                    } else {
