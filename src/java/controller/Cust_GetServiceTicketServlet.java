 ...
 * @author Admin
 */
public class Cust_GetServiceTicketServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (user != null) {
                ...
                ArrayList<ServiceTicket> listSt = std.getServiceTicket("" + user.getCustid());
                if (listSt != null && !listSt.isEmpty()) {
                    s.setAttribute("LIST_SERVICETICKET", listSt);
                    request.getRequestDispatcher("MainServlet?action=custDashboard").forward(request, response);
                } else {
                    request.setAttribute("ERROR", "Customer doesn't have Service ticket!");
                    request.getRequestDispatcher("MainServlet?action=custDashboard").forward(request, response);
                }
            } else {
