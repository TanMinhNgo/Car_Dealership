 ...
 * @author Admin
 */
public class Sale_CreateSt1Servlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if(sale != null) {
                ...
                int result = std.createServiceTicket(custid, carid, datereceived, datereturned);
                if(result > 0) {
                    ServiceTicket newTicket = std.getNewServiceTicket();
                    s.setAttribute("SERVICETICKET_NEW", newTicket);
                    response.sendRedirect("MainServlet?action=sale_createStPage1");
                } else {
                    request.setAttribute("ERROR", "Something wrong. Try again!");
                    request.getRequestDispatcher("MainServlet?action=sale_createStPage").forward(request, response);
                }
            } else {
