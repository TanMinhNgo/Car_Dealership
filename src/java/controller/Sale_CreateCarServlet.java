 ...
 * @author Admin
 */
public class Sale_CreateCarServlet extends HttpServlet {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (sale != null) {
                if(serialnumber != null && model != null) {
                    ...
                    int result = cd.createCar(serialnumber, model, year, colour);
                    if (result > 0) {
                        ...
                        request.getRequestDispatcher("MainServlet?action=sale_createCarPage").forward(request, response);
                    } else {
